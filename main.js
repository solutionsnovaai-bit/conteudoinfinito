/* ─────────────────────────────────────────────────────────────────────────────
   ...
   GSAP 3.12.5 + ScrollTrigger + Lenis
───────────────────────────────────────────────────────────────────────────── */
...
function runCurtain(onComplete) {
  ...
  const letters = document.querySelectorAll('.cl');

  // Mobile: skip the letter-split, just a quick fade (lighter)
  if (isMobile()) {
    gsap.set(letters, { opacity: 1, yPercent: 0, rotate: 0 });
    gsap.to(curtainEl, {
      opacity: 0, duration: .6, ease: 'power2.out', delay: .15,
      onComplete: () => { curtainEl.style.display = 'none'; onComplete(); }
    });
    return;
  }

  // Curtain is position:fixed z-index:10000 — covers page, no scroll lock needed
  const tl = gsap.timeline({
    onComplete,
    onInterrupt: onComplete  // fire even if timeline is interrupted
  });

  tl.to(letters, { yPercent: 0, rotate: 0, opacity: 1, duration: .55, stagger: .1, ease: 'power3.out' })
    .to('.curtain__bar',     { width: '100%', duration: 1.05, ease: 'power2.inOut' }, '<.12')
    .to('.curtain__letters', { scale: 1.08, filter: 'blur(14px)', opacity: 0, duration: .4, ease: 'power2.in' }, '+=.18')
    .to('.curtain__panel--top',    { yPercent: -100, duration: .85, ease: 'expo.inOut' }, '<')
    .to('.curtain__panel--bottom', { yPercent:  100, duration: .85, ease: 'expo.inOut' }, '<')
    .call(() => { curtainEl.style.display = 'none'; });
}
...
function initParticles() {
  ...
  const COLS   = ['rgba(234,240,250,', 'rgba(9,204,244,', 'rgba(235,190,80,'];
  ...
  function spawn() {
    ...
    return {
      ...
      color: COLS[ci], life: 0,
      maxLife: 180 + Math.random() * 220
    };
  }
  function tick() {
    raf = requestAnimationFrame(tick);
    if (window.__particlesActive === false) return;   // paused off-viewport
  ...
  // Pause the particle loop while the hero is out of view
  window.__particlesActive = true;
  const hero = document.querySelector('.hero');
  if (hero && 'IntersectionObserver' in window) {
    new IntersectionObserver(
      entries => { window.__particlesActive = entries[0].isIntersecting; },
      { threshold: 0 }
    ).observe(hero);
  }
  tick();
...
function heroMouseParallax() {
  ...
  hero.addEventListener('mousemove', e => {
    ...
    gsap.to(bg, { x: x * -10, y: y * -7, duration: 1.2, ease: 'power2.out', overwrite: 'auto' });
  });
}
...
async function initScrub() {
  ...
  // Smart preload: first, mid, last immediately — rest in idle
  ...
  function draw(frameIndex) {
    ...
    if (idx === lastFrame) return;  // dirty-flag: skip identical redraws
    lastFrame = idx;
...
/* ─── DEMO (link direto — sem JS necessário) ────────────────────────────────── */
// A thumbnail é um <a target="_blank"> — nenhum JS necessário
