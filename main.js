...
async function initScrub() {
  ...
  // Carrega só o primeiro frame agora — suficiente pro draw inicial
  const first = new Image();
  first.src = BASE + '001.webp';
  ...
  function draw(frameIndex) {
    ...
    if (idx === lastFrameIdx) return; // dirty-flag: não redesenha frame igual
    lastFrameIdx = idx;
    const img = frames[idx] || frames[0]; // fallback ao frame 0 se ainda não carregou
  ...
  // Carrega restante em lotes de 8 via requestIdleCallback
  function loadFrames() {
    ...
    let i = 1;
    function loadBatch() {
      const end = Math.min(i + 8, FRAME_COUNT);
      for (; i < end; i++) {
        const idx = i;
        const img = new Image();
      ...
      if (i < FRAME_COUNT) idle(loadBatch, { timeout: 1000 });
      else ScrollTrigger.refresh();
    }
    idle(loadBatch, { timeout: 500 });
  }
  ...
  // Dispara o carregamento quando usuário está ~800px acima da seção
  ScrollTrigger.create({
