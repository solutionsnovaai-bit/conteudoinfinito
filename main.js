...
// Dispositivos com < 4GB de RAM desativam animações secundárias
const LOW_END  = (navigator.deviceMemory || 4) < 4;
...
function initParticles() {
  ...
  // LOW_END → 8 partículas; mobile → 15; desktop → 30
  const PARTICLE_COUNT = LOW_END ? 8 : (window.innerWidth <= 860 ? 15 : 30);
...
function heroScrollExit() {
  if (REDUCE_MOTION || LOW_END) return;
...
function initMarquees() {
  ...
  document.querySelectorAll('.marquee-wrap').forEach(wrap => {
    ...
    ScrollTrigger.create({
      ...
      onUpdate(self) {
        if (!self.isActive || LOW_END) return;
...
function initStickyCTA() {
  ...
  window.addEventListener('scroll', () => {
    ...
    // Esconde quando a seção CTA Final entra na viewport
    const nearEnd = ctaFinal
      ? ctaFinal.getBoundingClientRect().top < window.innerHeight
      : false;
    cta.classList.toggle('visible', pastHero && !nearEnd);
  }, { passive: true });
}
